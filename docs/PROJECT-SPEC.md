...
> 1. Entry created — "Credit entry added for Crystal Mews Body Corporate, Amount: R 2 850" — Justin Justin, 2026-03-25T08:20:00
...
> 4. Allocated to invoice — "Linked to INV-2026-0001, Unit: A01" — Justin Justin, 2026-04-02T10:15:00
...
> **Example data — All Users tab (25 users):**
>
> **Internal (5):** Justin Justin (Admin, Active), Julian Tabona (Admin, Active), Thabo Ndlovu (Portfolio Manager, Active), Lerato Pillay (Financial Controller, Active), Naledi Khumalo (Portfolio Assistant, Active)
>
> **External (20):** Kgomotso Tlokweng (Trustee / Director, Active), Badisa Morokotso (Trustee / Director, Active), Boitumelo Molefe (Owner, Active), Gaositwe Sekgwele (Owner, Active), Keagile Ramotshabi (Tenant, Active), Masa Plumbing Services (Contractor, Active), Phenyo Electrical Works (Contractor, Active), and 13 more
...
- Fields (2-column grid):
  - Full Name (text input, e.g., "Justin Justin")
...
| MySQL | 8.0 | Primary database (Docker container, both local & production) |
| Redis | 7 | Queue driver, cache backend, sessions |
| AWS S3 | N/A | File and document storage |
| AWS ECR | N/A | Private Docker image registry (for app + nginx images) |
| AWS EC2 | t3.small | Production compute (single-node Docker host running the full stack) |
...
| Laravel Horizon | 5.x | Queue worker manager + dashboard for Redis-backed queues |
...
- Tests run against an in-memory SQLite database (configured in `phpunit.xml`) — fast, isolated, zero setup
- Use `RefreshDatabase` trait on all feature tests
- CI runs the full Pest suite on every push/PR via GitHub Actions before any deploy can proceed
...
| `DemoUsersSeeder` | 5 internal staff: 2 company admins, 1 portfolio manager, 1 financial controller, 1 portfolio assistant |
| `DemoExternalUsersSeeder` | 20 external users: 5 trustees, 5 owners, 5 tenants, 5 contractors |
| `DemoEstatesSeeder` | 6 demo estates with units, owners, tenants, invoices, and cashbook entries |
...
| Role | Email | Name |
|---|---|---|
| Company Admin | `justin@boldmarkprop.co.za` | Justin Justin |
| Company Admin | `brandontabona@gmail.com` | Julian Tabona |
...
**External Users (20 — all password: `password`):**

| Role | Email | Name |
|---|---|---|
| Trustee | `k.tlokweng@gmail.com` | Kgomotso Tlokweng |
| Trustee | `b.morokotso@hotmail.com` | Badisa Morokotso |
| Trustee | `t.nkosi@yahoo.com` | Thandiwe Nkosi |
| Trustee | `o.sekgololo@gmail.com` | Omphemetse Sekgololo |
| Trustee | `k.gaefele@gmail.com` | Kedibonye Gaefele |
| Owner | `boi.molefe@gmail.com` | Boitumelo Molefe |
| Owner | `g.sekgwele@gmail.com` | Gaositwe Sekgwele |
| Owner | `m.gaborone@yahoo.com` | Motlalepula Gaborone |
| Owner | `k.ntlo@outlook.com` | Kelapile Ntlo |
| Owner | `d.phele@gmail.com` | Dimakatso Phele |
| Tenant | `k.ramotshabi@gmail.com` | Keagile Ramotshabi |
| Tenant | `n.keorapetse@gmail.com` | Naledi Keorapetse |
| Tenant | `t.gaokgakala@yahoo.com` | Tumisang Gaokgakala |
| Tenant | `e.segolodi@gmail.com` | Emang Segolodi |
| Tenant | `b.setihapi@hotmail.com` | Boikhutso Setihapi |
| Contractor | `info@masaplumbing.bw` | Masa Plumbing Services |
| Contractor | `phenyo.elec@gmail.com` | Phenyo Electrical Works |
| Contractor | `admin@kgomoconstruct.bw` | Kgomo Construction Ltd |
| Contractor | `titi.clean@gmail.com` | TiTi Cleaning Services |
| Contractor | `ops@bwsecurity.bw` | Botswana Security Solutions |

**Super Admin (system-level — not a demo tenant user):**

| Role | Email | Name | Password |
|---|---|---|---|
| Super Admin | `super@optimumquality.co.za` | Optimum Quality Admin | `changeme-in-production` (env: `SUPER_ADMIN_PASSWORD`) |

> **Start with Company Admin** (`justin@boldmarkprop.co.za` / `password`) — this account has full access to all features.
...
The entire production stack runs on a **single AWS EC2 instance** (t3.small in `eu-west-2`, London)
using Docker Compose. Vue 3 frontend is built at image-build time and baked into the nginx
image — there is no separate frontend hosting. All services run as containers behind a single
nginx reverse proxy that terminates TLS via Let's Encrypt.
...
```
                  ...
                  └─────────────────┬───────────────┘
                                    ...
                                    ▼
┌───────────────────────────────────────────────────────────────────┐
│  AWS EC2 (t3.small, Ubuntu 24.04, eu-west-2)                      │
│  Elastic IP, Docker Compose stack                                 │
│                                                                   │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │ nginx container (Nginx + Let's Encrypt + Vue static files)   │ │
│  │   - / → serves Vue 3 SPA from /var/www/html/public-web       │ │
│  │   - /api/* → FastCGI to PHP-FPM (app:9000)                   │ │
│  │   - /horizon → FastCGI to PHP-FPM                            │ │
│  │   - /webhooks/* → FastCGI to PHP-FPM                         │ │
│  └────────────────────────┬─────────────────────────────────────┘ │
│                           │                                       │
│  ┌────────────────────────▼─────────────────────────────────────┐ │
│  │ app container (PHP-FPM 8.3 + Laravel 13)                     │ │
│  │ horizon container (queue workers — same image, different CMD)│ │
│  │ scheduler container (php artisan schedule:work — same image) │ │
│  └─────┬─────────┬─────────────────────────────────────────────┬┘ │
│        │         │                                             │  │
│  ┌─────▼─────┐  ┌▼────────────┐                                │  │
│  │ db        │  │ redis       │                                │  │
...
│                                                                │  │
│  ┌─────────────────────────────────────────────────────────────▼┐ │
│  │ certbot container (Let's Encrypt cert auto-renewal, 12h loop)│ │
│  └──────────────────────────────────────────────────────────────┘ │
└──────────────┬──────────────────┬──────────────────┬──────────────┘
               │                  │                  │
               ▼                  ▼                  ▼
       ┌───────────────┐  ┌───────────────┐  ┌───────────────┐
       │  AWS S3       │  │  Resend       │  │  Pusher       │
       │  - uploads    │  │  Transactional │  │  Real-time    │
       │  - db-backups │  │  email         │  │  events       │
       └───────────────┘  └───────────────┘  └───────────────┘
```
...
|---|---|---|
| `nginx` | ECR: `boldmark-nginx` | TLS termination, reverse proxy, serves baked-in Vue SPA |
| `app` | ECR: `boldmark-app` | PHP-FPM 8.3 + Laravel 13 API |
| `horizon` | ECR: `boldmark-app` (CMD override) | Redis queue worker manager + `/horizon` dashboard |
| `scheduler` | ECR: `boldmark-app` (CMD override) | `php artisan schedule:work` — replaces host cron |
| `db` | `mysql:8.0` (Docker Hub) | Primary database, persistent volume `mysql_data` |
| `redis` | `redis:7-alpine` (Docker Hub) | Queue + cache + sessions, persistent volume `redis_data` |
| `certbot` | `certbot/certbot` (Docker Hub) | Renews Let's Encrypt certs in a 12-hour loop |
...
#### Foundation
- [ ] Laravel 13 project setup with MySQL 8 (Dockerised, parity dev↔prod)
- [ ] Vue 3 + Vite + Tailwind 4 frontend (baked into nginx image at build time)
...
|---|---|---|---|
| `context7` | `@upstash/context7-mcp` | Live library docs (Laravel, Vue, Tailwind, MySQL, AWS, etc.) | Claude Code + Cursor |
...
2. **Use context7** to look up current documentation for any library (Laravel 13, Vue 3, Tailwind 4, MySQL, AWS, etc.)
...
The full production `.env` is stored as the GitHub secret `PRODUCTION_ENV` and written to disk
on the EC2 instance during deploy. The template lives in `api/.env.example`.

| Service | .env Variable | Notes |
|---|---|---|
| App key | `APP_KEY` | Generate once with `php artisan key:generate --show` |
| App URL | `APP_URL` | `https://portal.boldmarkprop.co.za` in production |
| DB connection | `DB_CONNECTION` | Always `mysql` (local dev + production) |
| DB host | `DB_HOST` | `127.0.0.1` (local), `db` (production — Docker service name) |
| DB credentials | `DB_DATABASE`, `DB_USERNAME`, `DB_PASSWORD`, `DB_ROOT_PASSWORD` | Generate strong passwords for production |
| Redis | `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD` | `redis` host in production (Docker service name) |
| Queue / Cache / Session | `QUEUE_CONNECTION`, `CACHE_STORE`, `SESSION_DRIVER` | All `redis` in production |
| AWS Access Key ID | `AWS_ACCESS_KEY_ID` | Created by `scripts/aws-bootstrap.sh` (boldmark-deployer IAM user) |
| AWS Secret Access Key | `AWS_SECRET_ACCESS_KEY` | Created by `scripts/aws-bootstrap.sh` |
| AWS Region | `AWS_DEFAULT_REGION` | `eu-west-2` |
| AWS S3 uploads bucket | `AWS_BUCKET` | `boldmark-uploads-<account_id>` (created by bootstrap) |
| AWS S3 backups bucket | `BACKUP_S3_BUCKET` | `boldmark-db-backups-<account_id>` (used by `backup-db.sh`) |
| Pusher | `PUSHER_APP_ID`, `PUSHER_APP_KEY`, `PUSHER_APP_SECRET`, `PUSHER_APP_CLUSTER` | From Pusher dashboard |
| Resend | `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET` | MCP active for development |
| Mail | `MAIL_MAILER=resend`, `MAIL_FROM_ADDRESS`, `MAIL_FROM_NAME` | Domain `boldmarkprop.co.za` verified in Resend |

### GitHub Actions Secrets (CI/CD)

These secrets are configured in **Repository Settings → Secrets and variables → Actions**.
Most are output by `scripts/aws-bootstrap.sh` to `.aws-bootstrap-output.txt`.

| Secret | Source | Used By |
|---|---|---|
| `AWS_ACCESS_KEY_ID` | Output of `aws-bootstrap.sh` (deployer IAM user) | `build-images` job (push to ECR) |
| `AWS_SECRET_ACCESS_KEY` | Output of `aws-bootstrap.sh` | `build-images` job |
| `EC2_HOST` | Elastic IP from `aws-bootstrap.sh` | `deploy` job (SSH target) |
| `EC2_USER` | `ubuntu` (default for Ubuntu 24.04 AMI) | `deploy` job |
| `EC2_SSH_KEY` | Contents of `scripts/boldmark-ec2.pem` from `aws-bootstrap.sh` | `deploy` job |
| `EC2_SSH_PORT` | `22` (default) | `deploy` job |
| `PRODUCTION_ENV` | Full `.env` file contents (template printed by `aws-bootstrap.sh`) | `deploy` job — written to `/opt/boldmark/.env` on the server |
| `VITE_PUSHER_APP_KEY` | Pusher dashboard (public app key) | `build-images` job — baked into Vue bundle |
| `VITE_PUSHER_APP_CLUSTER` | `eu` (default) | `build-images` job |
...
- **Node.js:** Latest LTS (CI uses Node 20)
- **PHP:** 8.3+
- **Package Manager:** npm (frontend), Composer (backend)
- **Local database:** MySQL 8 in Docker (single command — see below)

#### Spin up local MySQL

```bash
docker run -d --name boldmark-mysql \
  -p 3306:3306 \
  -e MYSQL_ROOT_PASSWORD=secret \
  -e MYSQL_DATABASE=boldmark \
  -e MYSQL_USER=boldmark \
  -e MYSQL_PASSWORD=secret \
  -v boldmark_mysql_data:/var/lib/mysql \
...
| Layer | URL | Status |
|---|---|---|
| Frontend + API (single host) | https://portal.boldmarkprop.co.za | Served by nginx on EC2 — frontend at `/`, API at `/api/*` |
| Horizon dashboard | https://portal.boldmarkprop.co.za/horizon | Queue worker monitoring (auth required) |
| Health check | https://portal.boldmarkprop.co.za/up | Used by deploy script + uptime monitor |
| Email sender domain | noreply@boldmarkprop.co.za | Verified in Resend |

> **Architecture note:** Frontend and API share the same domain (no separate api subdomain).
...
| Layer | Tech | Notes |
|---|---|---|
| Compute | AWS EC2 t3.small (Ubuntu 24.04 LTS) | Single-node, `eu-west-2` (London) |
| Public IP | AWS Elastic IP | Stable IP across instance restarts; DNS A record points here |
| Container registry | AWS ECR (private) | Repos: `boldmark-app`, `boldmark-nginx` |
| Container orchestration | Docker Compose (no Kubernetes) | Single compose file: `docker-compose.yml` |
| Database | MySQL 8.0 (Docker container, persistent volume) | Backed up nightly to S3 |
| Cache / Queue / Sessions | Redis 7 (Docker container) | Used by Horizon |
| File storage | AWS S3 (`boldmark-uploads-<account_id>`) | Private bucket, AES256 encryption |
| DB backups | AWS S3 (`boldmark-db-backups-<account_id>`) | Daily 02:00 UTC cron, IA after 30d, expire after 365d |
| TLS | Let's Encrypt via certbot container | Auto-renewed every 12 hours |
| Real-time | Pusher (managed SaaS) | |
| Email | Resend (`boldmarkprop.co.za` domain verified) | |

### CI/CD Pipeline — GitHub Actions

Single workflow file: `.github/workflows/ci-cd.yml`. Three jobs:

1. **`test`** — runs on every push & PR
   - Sets up PHP 8.3 + Node 20
   - Installs Composer + npm dependencies (cached)
   - Builds the frontend with Vite (catches Vue/Vite errors *before* a deploy can happen)
   - Runs the full Pest suite in parallel against in-memory SQLite
   - Required to pass before any image build can start

2. **`build-images`** — runs only on push to `main` (or `workflow_dispatch`)
   - Authenticates with AWS ECR using deployer IAM credentials
   - Auto-creates ECR repos if missing (idempotent first deploy)
   - Builds two images using `docker/build-push-action@v6` with GHA layer cache
     - `boldmark-app` (PHP-FPM + Laravel + composer deps)
     - `boldmark-nginx` (Nginx + Vue static build)
   - Pushes both to ECR with two tags: commit SHA + `latest`
   - Prunes old ECR images, keeping the 5 most recent per repo

3. **`deploy`** — runs after `build-images`
   - SCPs `docker-compose.yml`, `nginx/`, `docker/` to `/tmp/boldmark-deploy` on EC2
   - SSHes in and:
     - Writes `/opt/boldmark/.env` from the `PRODUCTION_ENV` GitHub secret
     - Pulls the new images from ECR
     - Runs `docker/scripts/deploy.sh` which:
       - Restarts the app container with the new image
       - Waits up to 60s for the app health check to pass — **aborts deploy on failure**
       - Runs `php artisan migrate --force`
       - Recreates nginx, horizon, scheduler with new images
       - Warms config/route/view/event caches
       - Best-effort `https://portal.boldmarkprop.co.za/up` check
   - Post-deploy `docker image prune` + builder prune to free disk

### One-Time Bootstrap (run once, then forget)

| Step | Command | What it does |
|---|---|---|
| 1. AWS infra | `bash scripts/aws-bootstrap.sh` (with AWS root creds in env) | Creates IAM deployer user, ECR repos, S3 buckets, EC2 key pair, security group, EC2 instance, Elastic IP. Outputs all GitHub secrets. |
| 2. EC2 OS setup | SSH to instance, run `sudo bash /tmp/setup-ec2.sh` | Installs Docker, AWS CLI, certbot, sets up `/opt/boldmark`, schedules nightly backup cron |
| 3. DNS | Point `portal.boldmarkprop.co.za` A record at the Elastic IP | DNS propagation usually 5-15 min |
| 4. SSL cert | One-time `certbot certonly --standalone -d portal.boldmarkprop.co.za` (instructions in `setup-ec2.sh` output) | Issues Let's Encrypt cert; certbot container then auto-renews |
| 5. GitHub secrets | Paste the values from `scripts/.aws-bootstrap-output.txt` into Repo Settings → Secrets → Actions | Required for the `build-images` and `deploy` jobs |
| 6. First deploy | `git push origin main` | Triggers test → build → deploy |
| 7. First-deploy seed | After step 6 succeeds, SSH in and run `sudo bash /opt/boldmark/docker/scripts/first-deploy.sh` | Seeds roles/permissions, creates super admin, installs Passport keys |

### Rollback Procedure

If a deploy goes bad and `/up` returns errors:

```bash
# SSH in:
ssh -i scripts/boldmark-ec2.pem ubuntu@<elastic-ip>

# Find the previous good image SHA in ECR:
aws ecr describe-images --repository-name boldmark-app --region eu-west-2 \
  --query 'sort_by(imageDetails,& imagePushedAt)[-2:].imageTags' --output table

# Rollback by pinning the image tag in /opt/boldmark/.deploy.env:
PREV_SHA=<the-previous-sha>
ECR=<account-id>.dkr.ecr.eu-west-2.amazonaws.com
sudo tee /opt/boldmark/.deploy.env > /dev/null <<EOF
...
If migrations were applied that broke things, restore the latest backup:
```bash
LATEST=$(aws s3 ls s3://boldmark-db-backups-<account-id>/database/ --region eu-west-2 | sort | tail -1 | awk '{print $4}')
aws s3 cp s3://boldmark-db-backups-<account-id>/database/$LATEST /tmp/$LATEST --region eu-west-2
gunzip /tmp/$LATEST
docker compose -f /opt/boldmark/docker-compose.yml exec -T db mysql -uroot -p$DB_ROOT_PASSWORD boldmark < /tmp/${LATEST%.gz}
```
...
| What | Where it lives |
|---|---|
| Production source of truth | GitHub `main` branch + ECR `:latest` images |
| Deployment trigger | `git push origin main` |
| Production .env | GitHub secret `PRODUCTION_ENV` (written to `/opt/boldmark/.env` on deploy) |
| Logs | `docker compose logs -f` on EC2; structured app logs in `/opt/boldmark/storage/logs` (volume) |
| Database backups | `s3://boldmark-db-backups-<account-id>/database/<timestamp>.sql.gz` (daily 02:00 UTC, 365-day retention) |
| Uploaded files | `s3://boldmark-uploads-<account-id>/` (private bucket) |
| SSL certs | `/etc/letsencrypt/live/portal.boldmarkprop.co.za/` on EC2 (volume) |
| Passport keys | `/var/www/html/storage/passport/` named volume (persists across deploys) |
| MySQL data | `mysql_data` named volume on EC2 |
| Redis data | `redis_data` named volume on EC2 |
...
| 2026-05-04 | **Database & deployment overhaul.** Removed Supabase/PostgreSQL entirely; standardised on **MySQL 8** for both local dev and production. Removed `pgsql` and `sqlsrv` connection blocks from `config/database.php`. Local dev now runs MySQL in a Docker container (port 3306) instead of the external PostgreSQL container. **Production architecture finalised:** AWS EC2 (t3.small, eu-west-2) running full Docker Compose stack — nginx + app (PHP-FPM) + horizon + scheduler + db (MySQL 8) + redis + certbot. Vue 3 frontend baked into the nginx image at build time (no separate Vercel deployment for the production stack). **CI/CD pipeline rewritten** in `.github/workflows/ci-cd.yml`: three-stage pipeline (test → build-images → deploy) using `docker/build-push-action@v6` with GHA cache, auto-creates ECR repos, prunes old images, deploys to EC2 via SSH with health-check abort. **AWS bootstrap automated** via `scripts/aws-bootstrap.sh` (runs AWS CLI in Docker, no local install required) — provisions IAM deployer user, ECR repos, S3 buckets, EC2 key pair, security group, EC2 instance, Elastic IP, and outputs every GitHub secret needed. EC2 setup script (`docker/scripts/setup-ec2.sh`) made idempotent with built-in certbot install and nightly DB backup cron. Section 15 (Architecture Overview) and Section 20 (Environment & Infrastructure) rewritten in full. New file: `DEPLOYMENT.md` (end-to-end runbook). | Julian Tabona / Claude Code |
