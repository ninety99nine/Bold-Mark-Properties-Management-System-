...
<head>
  <meta charset="UTF-8" />
  <title>Customer Statement — {{ $customerCode }}</title>
  @php
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page { padding: 28px 34px 210px 34px; }
    ...
    .txns a { color: #2f6fb0; text-decoration: underline; }

    /* Footer block (ageing + banking) pinned to bottom */
    ...
    .bank-row { width: 100%; margin-top: -1px; }
    .bank-cell { border: 1px solid #cfcfcf; padding: 10px 12px; font-size: 9px; line-height: 1.7; vertical-align: top; }
    .bank-cell .h { font-weight: bold; margin-bottom: 4px; }
  ...
  <table class="w100">
    <tr>
      ...
      <td class="cust-right" style="width: 50%;">
        <span class="bold">Unit No {{ $unit->unit_number }}</span><br>
        @if($customerCode)Reference: {{ $customerCode }}<br>@endif
    ...
    <tbody>
      @foreach($rows as $r)
        <tr>
          ...
          <td>
            @if($r['invoice_id'])
              <a href="{{ $frontendUrl }}/billing/invoices/{{ $r['invoice_id'] }}">{{ $r['description'] }}</a>
            @else
              {{ $r['description'] }}
            @endif
          </td>
...
{{-- Footer: ageing + banking + total due --}}
<div class="footer-block">
  ...
  <table class="bank-row">
    <tr>
      <td class="bank-cell" style="width: 37%;">
