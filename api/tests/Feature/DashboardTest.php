...
it('dashboard data is scoped to the authenticated user tenant', function () {
    ...
    Unit::factory()->count(5)->create(['estate_id' => $estateA->id, 'organization_id' => $userA->organization_id]);

    $userB   = adminUser(); // different tenant
    $estateB = Estate::factory()->create(['organization_id' => $userB->organization_id]);
    Unit::factory()->count(3)->create(['estate_id' => $estateB->id, 'organization_id' => $userB->organization_id]);
