...
const occupantId = computed(() => route.params.occupantId)
...
const invoiceForm       = ref({ ledgerId: '', billingPeriod: '', dueDate: '', amount: '' })
...
async function fetchData() {
  ...
  try {
    // 1. Fetch the unit occupant record (loads unit.community in the backend)
    const { data: occupantResp } = await api.get(
      `/communities/${communityId.value}/units/${unitId.value}/occupants/${occupantId.value}`
    )
    occupant.value = occupantResp.data

    // 2. Fetch invoices billed to this specific occupant
    const { data: invResp } = await api.get('/invoices', {
      params: {
        unit_id:        unitId.value,
        billed_to_type: 'occupant',
        billed_to_id:   occupantId.value,
        _per_page:      100,
      },
    })
    ...
    error.value = 'Failed to load occupant data. Please try again.'
    console.error(err)
  } finally {
    loading.value = false
  }
...
async function fetchLedgers() {
  try {
    const { data } = await api.get('/ledgers', { params: { is_active: 1, _per_page: 100 } })
...
onMounted(() => { fetchData(); fetchLedgers() })

// ── Computed helpers ──────────────────────────────────────────────────
const isCurrent     = computed(() => occupant.value?.is_active === true)
const unitNumber    = computed(() => occupant.value?.unit?.unit_number ?? '—')
const communityName    = computed(() => occupant.value?.unit?.community?.name ?? '—')
const communityRouteId = computed(() => occupant.value?.unit?.community_id ?? communityId.value)
...
// Outstanding balance for past occupants (Lease Details card)
...
const occupantOutstanding = computed(() =>
  invoices.value
...
const occupantCreditOnAccount = computed(() =>
  payments.value
    .filter(p => !p.invoice_id)
    .reduce((s, p) => s + Number(p.amount ?? 0), 0)
)
const occupantBalance        = computed(() => occupantCreditOnAccount.value - occupantOutstanding.value)
const occupantNetOutstanding = computed(() => Math.max(0, occupantOutstanding.value - occupantCreditOnAccount.value))
...
// ── Navigation ────────────────────────────────────────────────────────
const { goBack } = useBack(computed(() => ({ name: 'unit-detail', params: { communityId: communityId.value, unitId: unitId.value } })))
...
function goToUnit() {
  const u = occupant.value?.unit
  if (!u) return
  router.push({ name: 'unit-detail', params: { communityId: u.community_id, unitId: u.id } })
}
...
async function savePayment() {
  ...
  try {
    ...
    fd.append('community_id',   occupant.value?.unit?.community_id ?? '')
...
const occupantLedgerOptions = computed(() =>
  ledgers.value
    .filter(ct => ct.applies_to === 'occupant' || ct.applies_to === 'either')
...
async function saveInvoice() {
  ...
  try {
    await api.post('/invoices', {
      unit_id:        unitId.value,
      ledger_id: invoiceForm.value.ledgerId,
      billed_to_type: 'occupant',
      billed_to_id:   occupantId.value,
...
async function uploadLeaseFile(file) {
  ...
  try {
    ...
    const { data } = await api.post(
      `/communities/${communityId.value}/units/${unitId.value}/occupants/${occupantId.value}/lease-document`,
      formData,
      { headers: { 'Content-Type': 'multipart/form-data' } }
    )
    occupant.value = { ...occupant.value, lease_document_url: data.data.lease_document_url, lease_document_name: data.data.lease_document_name }
...
async function doDeleteLeaseDocument() {
  ...
  try {
    ...
    occupant.value = { ...occupant.value, lease_document_url: null, lease_document_name: null }
...
async function downloadLeaseDocument() {
  try {
    const response = await fetch(occupant.value.lease_document_url)
    const blob = await response.blob()
    const url  = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href     = url
    link.download = occupant.value.lease_document_name ?? 'lease-agreement'
    ...
    const link = document.createElement('a')
    link.href     = occupant.value.lease_document_url
    link.download = occupant.value.lease_document_name ?? 'lease-agreement'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
...
function printLeaseDocument() {
  ...
  iframe.src = occupant.value.lease_document_url
  ...
  iframe.onload = () => {
    try {
      ...
      const w = window.open(occupant.value.lease_document_url)
      w?.print()
    }
    setTimeout(() => {
...
function onTemplateChange() {
  const name = occupant.value?.full_name ?? ''
  const tplMap = {
    payment_reminder: {
...
// ── Edit Occupant modal ─────────────────────────────────────────────────
const showEditOccupant  = ref(false)
const editOccupantSaving = ref(false)
const editOccupantError  = ref(null)
const editOccupantForm   = ref({
  full_name:        '',
  email:            '',
  secondary_emails: [],
  phone:            '',
...
function openEditOccupant() {
  editOccupantError.value = null
  editOccupantForm.value = {
    full_name:        occupant.value?.full_name        ?? '',
    email:            occupant.value?.email            ?? '',
    secondary_emails: [...(occupant.value?.secondary_emails ?? [])],
    phone:            occupant.value?.phone            ?? '',
    rent_amount:      occupant.value?.rent_amount      ?? '',
    lease_start:      occupant.value?.lease_start      ?? '',
    lease_end:        occupant.value?.lease_end        ?? '',
  }
  showEditOccupant.value = true
}

async function saveEditOccupant() {
  editOccupantError.value  = null
  editOccupantSaving.value = true
  try {
    const { data } = await api.put(
      `/communities/${communityId.value}/units/${unitId.value}/occupants/${occupantId.value}`,
      editOccupantForm.value,
    )
    occupant.value = { ...occupant.value, ...(data.data ?? data) }
    showEditOccupant.value = false
    success('Occupant details updated.')
  } catch (err) {
    editOccupantError.value = err?.response?.data?.message ?? 'Failed to save changes. Please try again.'
  } finally {
    editOccupantSaving.value = false
  }
}
</script>
...
<template>
  ...
  <!-- ── Found state ────────────────────────────────────────────────── -->
  <div v-else-if="occupant" class="space-y-6 pb-8">
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="flex-1">
        <div class="flex items-center gap-3">
          <h1 class="font-body font-bold text-2xl text-foreground">{{ occupant.full_name }}</h1>
          <span :class="['inline-flex items-center rounded-full px-2 py-px text-[10px] font-medium border gap-1 leading-tight', isCurrent ? 'bg-success/10 text-success border-success/20' : 'bg-muted text-muted-foreground border-border']">
            {{ isCurrent ? 'Current Occupant' : 'Past Occupant' }}
          </span>
        </div>
        <p class="text-sm text-muted-foreground">
          Unit {{ unitNumber }} ·
          <button class="hover:underline text-primary" @click="router.push({ name: 'community-detail', params: { id: communityRouteId } })">{{ communityName }}</button>
        </p>
      </div>
      <div class="flex gap-2">
        <AppButton variant="outline" @click="openEditOccupant">
      ...
      <div class="space-y-6">
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
          ...
          <div class="p-6 pt-0 space-y-3">
            <div class="flex items-start gap-3 text-sm">
              ...
              <div class="flex flex-col gap-1">
                <div class="flex items-center gap-2">
                  <span class="text-foreground">{{ occupant.email }}</span>
                  <span v-if="occupant.secondary_emails?.length" class="text-[10px] font-medium text-muted-foreground bg-muted px-1.5 py-0.5 rounded uppercase tracking-wide">Primary</span>
                </div>
                <span v-for="email in (occupant.secondary_emails ?? [])" :key="email" class="text-foreground">{{ email }}</span>
              </div>
            </div>
            <div class="flex items-center gap-3 text-sm">
              ...
              <span class="text-foreground">{{ occupant.phone ?? '—' }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
          ...
          <div class="p-6 pt-0 space-y-3">
            <div class="flex justify-between text-sm">
              ...
              <span class="text-foreground font-medium">{{ formatDate(occupant.lease_start) }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-muted-foreground">Lease End</span>
              <span class="text-foreground font-medium">{{ formatDate(occupant.lease_end) }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-muted-foreground">Monthly Rent</span>
              <span class="text-foreground font-medium">{{ formatCurrency(occupant.rent_amount) }}</span>
            </div>
            <!-- Outstanding balance for past occupants -->
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
          <div class="flex items-center justify-between p-6 pb-3">
            ...
            <label v-if="occupant.lease_document_url" class="cursor-pointer">
          ...
          <div class="p-6 pt-0">
            ...
            <div v-if="!occupant.lease_document_url">
            ...
            <div v-else class="space-y-3">
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-xs font-medium text-foreground truncate">{{ occupant.lease_document_name ?? 'Lease Agreement' }}</p>
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
          ...
          <div class="p-6 pt-0 space-y-2">
            ...
            >
              <div>
                <p class="text-sm font-semibold text-foreground">Unit {{ occupant.unit.unit_number }}</p>
                <p class="text-xs text-muted-foreground mt-0.5">{{ occupant.unit.community?.name ?? '—' }}</p>
                <p class="text-xs text-muted-foreground mt-0.5">
                  {{ occupant.rent_amount ? 'Rent  ' + fmtAmount(occupant.rent_amount) + ' / month' : '—' }}
                </p>
              </div>
          ...
          <div class="p-5">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p :class="['text-3xl font-bold font-body', occupantBalance < 0 ? 'text-destructive' : 'text-foreground']">
                  {{ fmtAmount(occupantBalance) }}
                </p>
              </div>
              <div v-if="occupantBalance < 0" class="px-3 py-1 rounded-full bg-destructive/10 text-destructive text-xs font-medium">
                In Arrears
              </div>
            </div>
          </div>
        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
          ...
          <div class="p-6 pt-0">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="py-3 px-2 text-foreground">{{ inv.billed_to_unit_occupant?.full_name ?? occupant.full_name }}</td>
            ...
            <div v-if="invoices.length > 0" class="mt-3 pt-3 flex items-center gap-0 text-sm flex-wrap">
              ...
              <template v-if="occupantCreditOnAccount > 0">
                <div class="w-px h-3.5 bg-border mr-4" />
                <div class="flex items-center gap-1.5 pr-4">
                  <span class="text-muted-foreground text-xs">Credit</span>
                  <span class="font-semibold text-success font-body">{{ fmtAmount(occupantCreditOnAccount) }}</span>
                </div>
              </template>
              <div class="w-px h-3.5 bg-border mr-4" />
              <div class="flex items-center gap-1.5 pr-4">
                <span class="text-muted-foreground text-xs">Outstanding</span>
                <span :class="['font-semibold font-body', occupantNetOutstanding > 0 ? 'text-destructive' : 'text-foreground']">{{ fmtAmount(occupantNetOutstanding) }}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  <div v-else class="flex flex-col items-center justify-center py-24 text-center">
    <p class="text-lg font-medium text-foreground">Occupant not found</p>
    <p class="text-sm text-muted-foreground mt-1">This occupant record may have been removed or does not exist.</p>
  ...
  <AppModal :show="showAddPayment" size="md" @close="showAddPayment = false">
    ...
    <div class="space-y-4">
      <div v-if="occupant" class="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
        <div class="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-primary text-sm font-bold shrink-0">
          {{ occupant.full_name.split(' ').map(n => n[0]).slice(0, 2).join('') }}
        </div>
        <div>
          <p class="text-sm font-medium text-foreground">{{ occupant.full_name }}</p>
          <p class="text-xs text-muted-foreground">Occupant · Unit {{ unitNumber }}</p>
        </div>
      </div>
      <div>
  ...
  <AppModal :show="showSendMessage" size="md" @close="showSendMessage = false">
    ...
    <div class="space-y-4">
      <div v-if="occupant" class="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
        ...
        <div>
          <p class="text-sm font-medium text-foreground">{{ occupant.full_name }}</p>
          <p class="text-xs text-muted-foreground">{{ occupant.email }} · Occupant</p>
        </div>
      </div>
      <div>
        <label class="text-sm font-medium text-foreground mb-1.5 block">Template</label>
        <AppSelect v-model="messageForm.template" :options="OCCUPANT_TEMPLATES" placeholder="Select a template..." @change="onTemplateChange" />
      </div>
  ...
  <AppModal :show="showCreateInvoice" size="md" @close="showCreateInvoice = false">
    ...
    <div class="space-y-4">
      ...
      <div v-if="occupant" class="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-foreground">{{ occupant.full_name }}</p>
          <p class="text-xs text-muted-foreground">Occupant · Unit {{ unitNumber }} · {{ communityName }}</p>
        </div>
        <span class="inline-flex items-center rounded-full px-2 py-px text-[10px] font-medium border bg-blue-50 text-blue-700 border-blue-200">Occupant</span>
      </div>

      <!-- Ledger -->
      <AppSelect v-model="invoiceForm.ledgerId" label="Ledger" :options="occupantLedgerOptions" placeholder="Select ledger..." required />
  ...
  <!-- ── Edit Occupant modal ──────────────────────────────────────────── -->
  <AppModal :show="showEditOccupant" size="md" @close="showEditOccupant = false">
    <template #header>
      <h3 class="text-base font-bold font-body flex items-center gap-2 text-foreground">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5">
          <path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/>
    ...
    <div class="space-y-4">
      <p v-if="editOccupantError" class="text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-md px-3 py-2">{{ editOccupantError }}</p>

      <AppInput v-model="editOccupantForm.full_name" label="Full Name" placeholder="e.g. Lisa Mokoena" required />

      <!-- Primary email -->
      <AppInput v-model="editOccupantForm.email" label="Primary Email" type="email" placeholder="e.g. lisa@email.com" required />

      <!-- Additional emails -->
      <div class="space-y-2">
        <label class="block text-sm font-medium">Additional Emails</label>
        <div v-for="(_, i) in editOccupantForm.secondary_emails" :key="i" class="flex gap-2 items-center">
          <AppInput
            v-model="editOccupantForm.secondary_emails[i]"
            type="email"
          ...
          <button
            ...
            @click="editOccupantForm.secondary_emails.splice(i, 1)"
          >
        ...
        <button
          ...
          @click="editOccupantForm.secondary_emails.push('')"
        >
      ...
      <AppInput v-model="editOccupantForm.phone" label="Phone Number" placeholder="e.g. +267 72 000 0000" />

      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-foreground mb-1.5">Lease Start</label>
          <AppDatePicker v-model="editOccupantForm.lease_start" placeholder="Select date..." />
        </div>
        <div>
          <label class="block text-sm font-medium text-foreground mb-1.5">Lease End</label>
          <AppDatePicker v-model="editOccupantForm.lease_end" placeholder="Select date..." />
        </div>
      </div>

      <AppInput v-model="editOccupantForm.rent_amount" label="Monthly Rent" type="number" placeholder="0.00" :prefix="countryStore.currencySymbol" :min="0" :max="9999999999.99" />
    </div>

    <template #footer>
      <AppButton variant="outline" :disabled="editOccupantSaving" @click="showEditOccupant = false">Cancel</AppButton>
      <AppButton variant="primary" :disabled="editOccupantSaving" @click="saveEditOccupant">
        <svg v-if="editOccupantSaving" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 animate-spin">
          <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
        </svg>
        {{ editOccupantSaving ? 'Saving...' : 'Save Changes' }}
      </AppButton>
    </template>
  </AppModal>
