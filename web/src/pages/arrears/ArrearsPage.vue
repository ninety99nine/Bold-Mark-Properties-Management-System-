...
function typeLabel(type) { return ESTATE_TYPE_CONFIG[type]?.label || type }
function typeBadgeClass(type) { return ESTATE_TYPE_CONFIG[type]?.badgeClass || 'bg-muted text-muted-foreground border-border' }
...
const filterFields = computed(() => {
  ...
  return [
    ...
    {
      key: 'estate_type', label: 'Estate Type',
      options: [
    ...
    { key: 'charge_type', label: 'Charge Type', options: chargeOptions },
  ]
})
...
async function fetchArrears() {
  ...
  try {
    ...
    const { data } = await api.get('/arrears', { params })
    units.value       = data.data ?? []
    summary.value     = data.summary ?? null
    estates.value     = data.estates ?? []
    chargeTypes.value = data.charge_types ?? []
...
const ownerBarOpts = computed(() => ({
  ...
  onClick: (_event, elements) => {
    ...
    if (owner?.unit_id && owner?.estate_id) {
      router.push(`/estates/${owner.estate_id}/units/${owner.unit_id}`)
    }
  },
  plugins: {
    legend: { display: false },
...
// 3 — Arrears by Charge Type (vertical bar)
const chargeTypeData = computed(() => {
  const ct = summary.value?.by_charge_type ?? {}
...
// 4 — Arrears by Estate Type (donut)
...
// 5 — Top Tenant Arrears (horizontal bar — reversed so longest at bottom)
const reversedTenants = computed(() => [...(summary.value?.top_tenant_arrears ?? [])].reverse())

const topTenantData = computed(() => ({
  labels: reversedTenants.value.map(d => `${d.unit_number} – ${d.name.length > 18 ? d.name.slice(0, 18) + '...' : d.name}`),
  datasets: [{
    data: reversedTenants.value.map(d => d.amount),
...
const topTenantOpts = computed(() => ({
  ...
  onClick: (_event, elements) => {
    if (!elements.length) return
    const tenant = reversedTenants.value[elements[0].index]
    if (tenant?.unit_id && tenant?.estate_id) {
      router.push(`/estates/${tenant.estate_id}/units/${tenant.unit_id}`)
    }
  },
  plugins: {
    legend: { display: false },
...
<template>
  <div class="space-y-6 pb-8">
    ...
    <div class="fixed top-14 left-60 right-0 bottom-0 z-50 flex items-center justify-center pointer-events-none">
      <div class="pointer-events-auto flex flex-col items-center gap-4 rounded-2xl border border-[#e8a040]/40 bg-white/95 px-10 py-8 shadow-2xl text-center max-w-sm">
        ...
        <div>
          ...
          <h2 class="font-bold text-lg text-[#1a2744] leading-snug">Arrears — Coming Soon</h2>
          ...
          <ul class="mt-4 space-y-2 text-left">
            <li class="flex items-start gap-2 text-sm text-slate-600">
              ...
              See every unit with overdue invoices across your entire portfolio
            </li>
            <li class="flex items-start gap-2 text-sm text-slate-600">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mt-0.5 shrink-0 text-[#e8a040]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
              Track total arrears, units in arrears and longest overdue accounts
            </li>
            <li class="flex items-start gap-2 text-sm text-slate-600">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mt-0.5 shrink-0 text-[#e8a040]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
              View arrears broken down by estate and estate type with visual charts
            </li>
            <li class="flex items-start gap-2 text-sm text-slate-600">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mt-0.5 shrink-0 text-[#e8a040]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
              Filter by estate, charge type and sort by highest arrears or longest overdue
            </li>
          </ul>
        </div>
      </div>
    ...
    <!-- ── Page heading ──────────────────────────────────────────────── -->
    <div>
      <h1 class="font-body font-bold text-2xl text-foreground">Arrears</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <template v-else>
        ...
        <AppStatCard
          ...
          :subtitle="`across ${summary?.estates_affected ?? 0} estates`"
        >
          <template #icon>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="w-[18px] h-[18px] text-destructive">
    ...
    <!-- ── Estates with most arrears (capped at 5) ──────────────────── -->
    <div v-if="allEstateCards.length" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      >
        ...
        <p class="text-2xl font-bold font-body text-destructive">{{ formatCurrency(estate.overdue_total) }}</p>
        <p class="text-xs text-muted-foreground">{{ estate.units_count }} {{ estate.units_count === 1 ? 'unit' : 'units' }} overdue</p>
      </div>
      <button
        v-if="hasMoreEstates"
    ...
    <div class="rounded-lg border bg-card shadow-sm">
      ...
      <div class="p-4 pb-0">
        <AppTableToolbar
          search-placeholder="Search units, owners, estates..."
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-border">
              ...
              <th class="text-left font-medium text-muted-foreground px-4 py-3 text-xs uppercase tracking-wider">Owner / Tenant</th>
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      ...
                      :class="['inline-flex items-center rounded-full px-1.5 py-px text-[9px] font-medium border leading-tight', typeBadgeClass(unit.estate.type)]"
                    >
    ...
    <template v-else-if="hasChartData">
      ...
      <!-- Row 1: Duration + Estate Type donut -->
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="rounded-lg border bg-card shadow-sm">
          <div class="px-6 pt-5 pb-2">
            <h3 class="font-body font-semibold text-base text-foreground">Arrears by Estate Type</h3>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="rounded-lg border bg-card shadow-sm">
          ...
          <div class="px-6 pb-6">
            <div class="h-[260px]">
              <Bar :data="topTenantData" :options="topTenantOpts" />
            </div>
          </div>
        </div>
      ...
      <!-- Row 3: By Charge Type (full width) -->
      <div class="rounded-lg border bg-card shadow-sm">
        <div class="px-6 pt-5 pb-2">
          <h3 class="font-body font-semibold text-base text-foreground">Arrears by Charge Type</h3>
          <p class="text-xs text-muted-foreground mt-0.5">Overdue amounts broken down by billing category</p>
        </div>
        <div class="px-6 pb-6">
          <div class="h-[280px]">
            <Bar :data="chargeTypeData" :options="chargeTypeOpts" />
          </div>
        </div>
      </div>
    ...
    <!-- ── All Estates Modal ──────────────────────────────────────────── -->
    <AppModal :show="showAllEstatesModal" title="Arrears by Estate" size="md" @close="showAllEstatesModal = false">
      <div class="space-y-2 -mx-2">
        ...
        >
          ...
          <p class="text-lg font-bold font-body text-destructive whitespace-nowrap ml-4">{{ formatCurrency(estate.overdue_total) }}</p>
        </div>
      </div>
    </AppModal>
