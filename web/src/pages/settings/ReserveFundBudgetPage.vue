<!--
  ReserveFundBudgetPage — routed at /settings/finance/reserve-fund-budget.

  Standalone page mounting the SAME shared ReserveFundBudgetTab component used by the
  Financial page's "Reserve Fund Budget Setup" tab. Reached from the sidebar it defaults
  to the "Setup Reserve Fund Budget" sub-tab (the Financial-page tab defaults to the
  report sub-tab). Includes the Financial Year / Budget Period selector + indicator.
-->
<script setup>
  ...
  <div class="space-y-6 pb-8">
    <div>
      <p class="text-sm text-muted-foreground">
        ...
        <span class="text-foreground font-medium">Reserve Fund Budget</span>
      </p>
      <h1 class="font-body font-bold text-2xl text-foreground mt-1">Reserve Fund Budget</h1>
      <p class="text-sm text-muted-foreground">
        Set up the reserve fund budget and review budget vs actual
    ...
    <div v-if="!cid" class="rounded-lg border border-border bg-white p-8 text-center text-muted-foreground">
      Select a community from the top bar to manage its reserve fund budget.
    </div>
    ...
    <template v-else>
      ...
      <ReserveFundBudgetTab :year="selectedYear" default-sub-tab="setup" />
    </template>
  </div>
</template>
