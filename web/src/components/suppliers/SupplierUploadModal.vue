<template>
  <AppModal :show="show" size="lg" @close="handleClose">
    <template #header>
      <h3 class="w-full text-center text-base font-bold text-foreground">Upload Suppliers</h3>
    </template>

    <!-- Upload step -->
    <div v-if="step === 'upload'">
      <p class="mb-4 text-sm text-[#3f4657]">
        To upload a supplier 'batch', please download and use the template provided below:
      </p>
      <div class="mb-5">
        <button type="button" class="inline-flex items-center gap-2 text-sm font-semibold text-[#2f6fb0] hover:underline disabled:opacity-50" :disabled="downloading" @click="downloadTemplate">
          <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><path d="M7 10l5 5 5-5" /><path d="M12 15V3" /></svg>
    ...
    <div v-else class="py-4 text-center">
      <div
        ...
        :class="result?.errors?.length ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'"
      >
        <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
      </div>
      <p class="mb-1 text-lg font-semibold text-foreground">{{ result?.created ?? 0 }} created, {{ result?.updated ?? 0 }} updated</p>
      <p v-if="result?.errors?.length" class="text-sm text-muted-foreground">{{ result.errors.length }} row{{ result.errors.length === 1 ? '' : 's' }} with errors</p>
      ...
      <div v-if="result?.errors?.length" class="mt-4 max-h-48 divide-y divide-border overflow-auto rounded border border-border text-left">
        <div v-for="(e, i) in result.errors" :key="i" class="px-3 py-2 text-xs text-muted-foreground">
          <span class="font-medium text-foreground">Row {{ e.row }}:</span> {{ e.message }}
        </div>
      </div>
...
async function downloadTemplate() {
  ...
  try {
    const res = await api.get('/suppliers/upload-template', { responseType: 'blob' })
    const url = URL.createObjectURL(res.data)
    const a = Object.assign(document.createElement('a'), { href: url, download: 'suppliers-upload-template.xlsx' })
...
async function runImport() {
  ...
  try {
    ...
    form.append('community_id', props.communityId)
    const res = await api.post('/suppliers/import', form)
...
function importedAny() {
  return (result.value?.created || 0) > 0 || (result.value?.updated || 0) > 0
}
function handleClose() {
  if (importedAny()) emit('imported')
  emit('close')
}
