<template>
  <AppModal :show="show" size="lg" @close="handleClose">
    <template #header>
      <h3 class="text-base font-bold text-[#1E2740] w-full text-center" style="font-family: 'DM Sans', sans-serif">Upload Units</h3>
    </template>

    <!-- ─── Upload step ─── -->
    <div v-if="step === 'upload'">
      <!-- Download existing unit file -->
      <button
        type="button"
      ...
      >
        ...
        {{ downloading ? 'Downloading…' : 'Download existing unit file here' }}
      </button>
      ...
      <ul class="space-y-2.5 mb-6 text-sm text-[#3f4657]">
        ...
        <li class="flex items-start gap-2"><span class="text-[#E8A040] mt-0.5">&#9888;</span> If the community you are uploading is a HOA, please enter the Erf No. in the "Section" column of the template file.</li>
        <li class="flex items-start gap-2"><span class="text-[#E8A040] mt-0.5">&#9888;</span> If the community you are uploading is a HOA, please enter the Street Name and No. in the "Door No" column.</li>
        <li class="flex items-start gap-2"><span class="text-[#E8A040] mt-0.5">&#9888;</span> An Email Address column can contain only one Email Address.</li>
        <li class="flex items-start gap-2"><span class="text-[#E8A040] mt-0.5">&#9888;</span> A Cell Number column can contain only one Cell Number.</li>
      </ul>
    ...
    <div v-else-if="step === 'result'" class="text-center py-4">
      ...
      <p class="text-lg font-semibold text-[#1E2740] mb-1">{{ result?.imported ?? 0 }} unit{{ (result?.imported ?? 0) === 1 ? '' : 's' }} processed</p>
      <p v-if="result?.error_count" class="text-sm text-[#717B99]">{{ result.error_count }} row{{ result.error_count === 1 ? '' : 's' }} had issues</p>
...
async function downloadExisting() {
  ...
  try {
    const res = await api.get(`/communities/${props.communityId}/units/export`, {
      params: { _format: 'xlsx', _limit: 'all' },
      responseType: 'blob',
    })
    let filename = 'unit export.xlsx'
    ...
    error.value = 'Could not download the existing unit file.'
  } finally {
...
async function runImport() {
  ...
  try {
    ...
    const parsed = await api.post(`/communities/${props.communityId}/units/bulk-import/parse`, form)
    const rows = parsed.data.rows ?? []
    if (!rows.length) { error.value = 'The file contains no data rows.'; return }
    const res = await api.post(`/communities/${props.communityId}/units/bulk-import`, { rows })
